...
| No em dash, emoji, or glossary-rejected synonym in tracked text; no voice tell on handbook pages, and no banned word in rubric TSVs or syllabi | `pre-commit`, staged files | `after-edit.mjs` on the edited file | `build`: `npm run check:prose` |
