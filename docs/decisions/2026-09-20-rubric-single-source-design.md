...
Status: implemented in #144; the TSV became Canvas's own CSV import format in #284, which also retired the browser extension, so read TSV below as the rubric file. Section 6's open decisions are resolved; the resolutions are recorded inline. Section 6.2's two unrendered directories were reversed by #259 (`2026-09-23-canvas-entry-model.md`): both are now rendered. The two survey pages' hand-written tables were reversed by #300: both render rubric CSVs.
...
**Descriptions that only restate the top band were dropped.** Moving every handbook criterion clause into field 2 produced 68 descriptions, of which **18 were near-identical to their own Exceeds band** (`incident-postmortem` row 1 was a character-for-character match). Rendered, the row said the same sentence twice. Review dropped five more: three that restated a band less exactly, and two from `repo-checkpoint` that described the page rather than the work, neither of which means anything in Canvas. **45 of the 75 criterion rows carry a description**; the other 30 are the 23 dropped plus 7 whose criterion name already said everything.
