...
- `individual-contribution/individual-contribution-rubric.csv`: the Half band names a contribution mode agreed in `CONTRIBUTING.md`, not the charter. Re-import on every Sprint Notes N: Individual Contribution entry.
- `project-handoff/project-handoff-rubric.csv`: the living-docs criterion names the requirements, design, and risk documents, not four living docs.
...
- `repo-checkpoint/repo-checkpoint-rubric.csv`: the Contribution traceability bands judge whether each student's work is attributable, code through reviewed PRs and other work where the charter says it lives, instead of whether every student met the PR-per-sprint norm. Re-import on both Repo Checkpoint entries.
