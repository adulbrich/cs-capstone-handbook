...
- **Outcome type.** Four [outcome types](/practicalities/types/) (FOSS, Research, Consultancy, New Product or Game) cover most projects and pick the shipping playbook, and a project that fits none of them is fine; a submission does not have to name one, because the team and the project partner agree it after teams are announced. Every project is [scored on who ran the software](/assignments/project-partner-evaluation/#spring), whatever its outcome type.
- **Student proposals.** A student may submit a project of any outcome type, but it needs a project partner or an approved mentor, and that mentor (or the instructor, when there is no outside mentor) acts as the project partner for the year, partner evaluations included. If it is accepted, the student and any teammates named in the proposal are assigned to it automatically; it may not appear in the public list at all.
...
Students bid on projects in a survey before the term starts, and the instruction team announces the teams in fall week 1 (the [schedule](/introduction/schedule/) has the week). We aim for teams of 3 to 4 students; real teams range from 2 to 5 depending on enrollment and project needs, and the instructors balance skills and preferences across all teams.
...
Your grade is based on the project you present at each milestone, with the same standards throughout. The later you pivot, the less runway you have; teams that wait until winter or spring to pivot rarely recover. If you suspect a dead end, say so in the next sprint note and TA check-in.
...
<LinkCard
  title="Project Types"
  description="Outcome type, origin, starting code, and constraints: what each one changes for your year."
  href="/practicalities/types/"
/>
