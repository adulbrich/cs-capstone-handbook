...
Every project commits to 2 or 3 metrics, and what makes a good one differs by category, so start with your own subsection below. Product and game teams can borrow from the PMF question, AARRR, and HEART in the [guide](/guides/planning/#success-metric-frameworks).
...
AARRR's activation and retention and HEART's task success are the usual picks; ProductPlan's [metrics list](https://assets.productplan.com/content/Product-Success-Metrics-by-ProductPlan.pdf) has more.

### Research

Your metric decides what counts as a result, so it is a one-way door: change it late and every earlier run stops being comparable.
...
Pair this with [Reproduce Your Baseline](/activities/design/#reproduce-your-baseline); a metric only means something next to a number you have reproduced.
...
In the workshop, the second half produces the same metrics list with an AI agent (a skill or a structured prompt) and sets it beside yours; the differences are the lesson, and either version can be the one you keep.
...
In the workshop, the second half produces the same critical path with an AI agent (a skill or a structured prompt) and sets it beside yours; the differences are the lesson, and either version can be the one you keep.
