...
Metrics you define and instrument early are exactly what the spring [Release and Metrics](/assignments/release/) assignment asks you to report on.
...
## Choose Your Evaluation Metric Before You Run Anything

<Badge text="Team Activity" variant="note" class="mb-6"/> <Badge text="Recommended" variant="tip" class="mb-6"/>

Commit to how you will measure success before you see any results, so you cannot be talked into whichever metric happens to look good. One hour, before the first real experiment.

- **Write the primary metric** and why it is right for the question, not merely easiest to compute.
- **Write the decision rule**: what value counts as success, what counts as failure, and what you do in each case.
- **Name the secondary metrics** you will report regardless of outcome, including the ones that could make you look bad.
- **State the comparison**: against which baseline, on which data split, over how many runs, with which seeds.
- **Commit it to the repository, dated**, before the experiment runs.

Choosing a metric after seeing results is the most common way honest people produce misleading work. It rarely feels like cheating. It feels like noticing that a different measure describes the finding better.

A good output is a dated, committed note stating the primary metric, the decision rule, the comparison, and the secondary metrics, written before any real run.

**Feeds:** the [Definition of Shipped](/assignments/definition-of-shipped/) metrics-and-instrumentation criterion, and the spring [Release and Metrics](/assignments/release/) report.
