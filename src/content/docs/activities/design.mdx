...
Draw the system your team is about to build, at the level of zoom your project partner can follow, and write one paragraph per box. One to two hours as a team, once in fall, then updated when the architecture changes.
...
- **Mark the risky parts**: which component is new to the team, which integration is unproven, and what you will spike first.
- **State scalability, security, and performance assumptions**: one line each, or "not a concern for this project" with the reason.

In the workshop, the second half produces the same architecture description with an AI agent (a skill or a structured prompt) and sets it beside yours; the differences are the lesson, and either version can be the one you keep.
...
<Badge text="Individual Activity" variant="success" class="mb-6"/> <Badge text="Workshop" variant="caution" class="mb-6"/>
...
In the workshop, the second half produces the same test plan with an AI agent (a skill or a structured prompt) and sets it beside yours; the differences are the lesson, and either version can be the one you keep.
