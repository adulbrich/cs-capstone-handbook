...
1. **Some decisions are hard to reverse.** Undoing them costs far more than getting them right. Those deserve your attention. Most other work does not.
2. **You may delegate only as far as your checks catch.** Trust without checks is recklessness. Trust with them is ordinary engineering. If you want to delegate more, build better checks first.
...
You are never required to use any of these tools. What matters is your judgment about where to delegate and how you verified the result, not which subscription you hold, and a student on a free tier who gates tightly and validates carefully is doing the harder work. Every activity below states its substitute for more limited tooling.
:::
...
A good output is a list where the hard-to-reverse decisions are few. If you classified eight of ten as hard to reverse, you have not made the distinction, you have just labeled everything important.
...
- **Close it from the top**: the highest-value check is always the one guarding the decision you cannot walk back.
...
Several of these *are* checks rather than conveniences: a separate architectural-review pass and a code-review pass are checks in their own right, and a plan-before-you-build skill is what turns "make it do X" into the kind of specification a design document can be written from.

:::caution[A skill is someone else's opinion, executed automatically]
Read a skill before you install it. Some encode strong workflow opinions that may not suit your project, and you own whatever they produce, and nobody reviewing it will care that a skill wrote it.
:::
...
That judgment is what your project partner is relying on, and it is the first thing an experienced reviewer probes for.
