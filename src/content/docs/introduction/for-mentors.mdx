---
title: For Mentors
description: No project idea? Our students still benefit from having an industry guide to get feedback and stay on track. Learn how to mentor without submitting a project.
sidebar:
  order: 4
---

import { LinkButton } from '@astrojs/starlight/components';

As an industry professional, you can make a meaningful impact on our students even without a project idea. Many students propose their own projects, and they need experienced mentors to guide them, provide feedback, and hold them accountable.

## Why Mentor?

Our students often come up with excellent project ideas on their own. These range from personal portfolio projects to open-source contributions to startup concepts. While these projects are often technically solid, students benefit enormously from:

- **External perspective**: An industry guide who can validate their direction and offer real-world feedback
- **Accountability**: Regular check-ins help students stay on track, contain scope creep, and avoid distractions
- **Career guidance**: Mentors can share their experience and help students make better technical and professional decisions
- **Network connections**: Introductions to potential collaborators, cofounders, users, employers, or resources

Not having a project idea shouldn't stop you from helping. Our students need mentors who can guide them through the process, ask tough questions, and help them deliver something they're proud of.

We are especially looking for people who shipped something recently. A mentor who built and launched a product three years ago is more useful to a senior than a vice president is, because the students can picture being them, and the advice comes from decisions still fresh. Recent alumni who started a company or shipped a real project are explicitly wanted, whatever the size of the outcome.

## What We Expect From You

We expect the same things from mentors as from project partners: availability from Fall through Spring, at least 1 hour per week with the team, clear expectations set together with the students, and a term-end evaluation of their work. The full list is on the [project partners page](/introduction/for-partners/#what-we-expect-from-you).

Your evaluations account for 25% of each student's grade, the same weight as each of the other three components. The [assignments overview](/assignments/introduction/#grade-architecture) has the full breakdown.

Two things are specific to mentoring:

- You don't need to be an expert in every technical detail. Your support, encouragement, and perspective are what matter most. Students are looking for guidance on how to work in a real-world project environment, and that's where your experience shines.
- Since student-proposed projects rarely involve NDAs, the teaching staff will usually have direct read access to the team's repository, so your evaluation can focus on direction, process, and professionalism rather than verifying every artifact.

## What You Can Expect From Us

As the instruction team, we will:

- match you with a student team based on your interests and their project needs;
- share the students' contact information and project details at the start of Fall term;
- provide rubrics and surveys for you to evaluate the team each term;
- be available if you have questions or concerns about the students or coursework;
- help you understand what's expected at each stage of the project, following the [assignments overview](/assignments/introduction/).

## How to Get Started

If you're interested in mentoring a team without submitting a project, we'd love to hear from you.

<div class="cta text-center">
  <LinkButton
    href="mailto:cs-46x-help@oregonstate.edu?subject=Mentor%20Interest%20-%20No%20Project"
    variant="primary"
    icon="email"
    iconPlacement="start"
    target="_blank"
  >
    Contact Us to Mentor
  </LinkButton>
</div>

When you reach out, please let us know:

- A bit about your background and industry experience
- The types of projects or areas you'd be interested in mentoring (e.g., web development, mobile apps, data science, machine learning, open source, etc.)
- Any preferences for team size or project scope

We'll follow up with you before Fall term starts to discuss next steps and match you with a suitable team.

## Have a Project Idea After All?

If you do have a project idea but just weren't sure if it was suitable, we encourage you to submit it anyway. We can help you refine it to make it a great fit for our students. 

<div class="cta text-center">
  <LinkButton
    href="https://eecs.engineering.oregonstate.edu/capstone/submission/pages/"
    variant="primary"
    icon="external"
    iconPlacement="start"
    target="_blank"
  >
    Submit a Project
  </LinkButton>
</div>
