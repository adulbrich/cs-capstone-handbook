...
The last line is your confidentiality rule, spelled out for the tools. The human-readable version belongs in the team's working agreement and says the same thing in the partner's terms:
...
Two published collections are worth reading, because reading a well-built skill is the fastest way to learn what a good one looks like:
...
- **Gate the hard-to-reverse decisions first.** Schema changes, auth, deployment config and credentials are where a blocked action is worth an inconvenience. Everything else can stay advisory.
...
- **Update it when it lies.** An instruction file that describes the architecture of three months ago actively misleads the tool. Re-read it whenever the architecture moves.
