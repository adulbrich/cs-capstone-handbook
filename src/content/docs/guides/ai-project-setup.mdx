...
Read this in the first sprint, once the repository exists and before the team has written much code with AI help. It is configuration, not practice: the [Generative AI guide](/guides/generative-ai/) has the habits, and this page has the files that make those habits cheap to follow.

The difference between a helpful AI assistant and a frustrating one is almost entirely about context. A tool with no knowledge of your project produces generic suggestions: the wrong framework, a second way of doing something you already do one way, a dependency nobody agreed to. The same tool, given your architecture, your conventions and your constraints, produces suggestions that fit.

Four things carry that context, and all four are files in your repository, which is the point: they are reviewed, versioned, and shared, so every teammate's tool behaves the same way. [agents.md](https://agents.md/) is the cross-tool convention for the first, and the [Model Context Protocol](https://modelcontextprotocol.io/) is the open standard behind the second.

Without them, each teammate prompts differently, gets different answers, and the codebase drifts toward inconsistency that nobody can point at a commit for.
