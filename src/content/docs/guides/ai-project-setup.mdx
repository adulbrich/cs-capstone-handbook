...
| CI and branch protection | On the pull request | Nothing |
...
Anyone can skip that with `--no-verify`, which is the reason for the third layer: the same checks run in CI, where nobody can skip them, and branch protection turns a red build into a blocked merge. The [workflow file](/guides/devops/#continuous-integration-ci) and the [repository settings](/guides/git-and-github/#protecting-the-main-branch) are covered in those guides.

Read the three as a trade between speed and authority. The agent hook is instant and advisory. CI is slow and final. Put the fast feedback where it saves you a round trip and the real gate where it cannot be skipped, and do not mistake one for the other.
...
<LinkCard
  title="Audit Your Safety Net"
  href="/activities/ai/#audit-your-safety-net"
  description="Measure the gap between how much your team delegates and how much your gates would actually catch."
/>
...
- **Test a gate by trying to break it.** Run the command the hook should block. A gate nobody has tripped on purpose is an assumption.
...
Hooks are the highest-leverage item on this page and the one teams reach for last. Three of them cover most of the damage a delegated agent can do to a repository. Past that the returns fall off fast: teams write elaborate policy hooks, trip over them twenty times a day, and quietly disable the lot in week 6. A gate that fires constantly gets removed, and then you have no gate.
