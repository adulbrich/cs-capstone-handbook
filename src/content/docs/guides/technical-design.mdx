...
- **Component diagram**: how components wire together into larger components or systems; the modular structure at a glance.
- **Package diagram**: the codebase grouped into packages, with the dependencies between them. Useful for spotting a dependency cycle before it costs you a refactor.
- **Deployment diagram**: physical or cloud topology of the running system, mapping artifacts (executables, containers, libraries) to nodes (servers, devices).
- **Composite structure diagram**: the internal parts of one classifier and how they collaborate. Rarely worth drawing by hand.

The [UML diagram reference](https://www.uml-diagrams.org/) covers the notation for each. Pick the diagrams that answer the questions your team is currently uncertain about. A sequence diagram for your authentication flow is more valuable than a complete UML suite nobody maintains. Never create a diagram just because a template requires it.
