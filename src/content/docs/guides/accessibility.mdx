...
description: "Automated gates, the manual pass, and a session with someone who uses assistive technology: three layers, cheapest first."
sidebar:
  order: 5
---

Read this in the sprint where a real interface first exists, not in the one before the Expo. Accessibility work is cheap while the markup is still being written and expensive once a layout is finished.
...
- **Gate in CI, not before the deadline.** A violation caught in a pull request costs minutes. The same violation found the week before the Expo costs a redesign.
...
Most capstone teams do none of this and nothing visible happens, because the graders, the partners and the teammates all use a mouse and a screen. That is exactly why it gets skipped: the cost of skipping it lands on people who are not in the room.

Automated tools are oversold and still worth running. [Deque's own audit data](https://www.deque.com/automated-accessibility-coverage-report/) puts axe-core at 57% of issues by volume, and counting WCAG success criteria instead gives a lower figure. A green automated run is not an accessible product; it is the mechanical half done.
