...
This guide is the playbook, one section per [project category](/practicalities/categories/). Each section says what each rung means for that category (the partner evaluation page holds the ladder itself), walks the prerequisites rung by rung, gives a term-by-term timeline, names the common blockers, and lists the evidence you should be collecting for the partner survey. Your [Definition of Shipped](/assignments/definition-of-shipped/) turns this playbook into a signed agreement; the [Repo Checkpoint gates](/assignments/repo-checkpoints/) enforce the intermediate milestones.

## FOSS

**On the ladder:** a pull request submitted is 70; reviewed and revised, 80; merged, 90; shipped in an upstream release, 100. Note what separates submitted from merged: not your code, but a maintainer's attention and approval. That is a resource you must cultivate, not a step you execute.
...
**On the ladder:** friends tried a deployed build is 70; a dozen strangers or a completed user study, 80; two dozen strangers, 90; sustained use, tens of heavy or hundreds of light users, 100. The jump from friends to a dozen strangers is the hard one: strangers cannot try software that is not deployed, and they will not arrive the week you need them.
...
**On the ladder:** a real-data run shown to your partner is 70; your partner's group ran it on real data and you fixed what they found, 80; someone outside the team reran the pipeline and got your numbers, 90; a stranger reproduced the result from the published artifact, or the paper is out, 100. Underneath every rung sits an unstated prerequisite: **data you are allowed to use** and **results someone else can reproduce**.
...
**On the ladder:** the partner tried a prototype is 70; their feedback addressed in a newer version, 80; the partner integrating it into production, 90; the partner running it in operations, 100. Read the top rungs carefully: they describe actions the **partner** takes, not you. Your job is to make those actions easy, safe, and early.
...
- "The partner will deploy it after the course ends" is the 80 rung wearing the 100 rung's costume. If it is not in their environment by spring, it is not shipped.
