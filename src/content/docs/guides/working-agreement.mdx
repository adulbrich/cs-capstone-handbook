...
Read this before writing your team's agreement, and when a norm nobody agreed on starts causing friction; it gives you what the agreement covers and how to revise it.
...
A working agreement supports psychological safety by making implicit expectations explicit. When a team member says "I thought we agreed to review PRs within a day," the agreement is where that expectation is either confirmed or revealed as an assumption that was never actually shared.

This is especially important on a capstone team, where teams form quickly, members may not know each other well, and there is no manager to set norms. The team has to define its own operating model, or one will emerge by default (and it usually will not be one anyone chose).
...
The Definition of Done (DoD) answers one question: *what does it mean for a task to be finished?* It comes from Scrum, where the [Scrum Guide](https://scrumguides.org/scrum-guide.html) makes it a formal commitment attached to the increment rather than a team preference, and the reason it is a commitment is that without one an increment cannot be inspected.
...
Name the stage your team is in first: the [Team Formation Strategies Assessment](/activities/teamwork/#team-formation-strategies-assessment) walks [Tuckman's forming-to-performing ladder](https://en.wikipedia.org/wiki/Tuckman%27s_stages_of_group_development), the 1965 model that named the storming phase teams are usually surprised by. A forming team needs explicit roles and response times more than a performing one does, and the agreement should say more where the team knows each other less.
...
## Roles (rotate at least every ten weeks)
| Role | Now | Owns |
|---|---|---|
...
A working agreement written in the first weeks and never revisited is a wasted artifact. The team that wrote it is not the same team three months later; the project has changed, the dynamics have shifted, and some norms have proven unrealistic.

Revisit it at the natural moments:

- **Start of each new phase of the project**: does it still reflect how the team actually works?
...
Version your working agreement in your repository. Being able to see how it evolved over the project is useful context, both for the team and for anyone evaluating how the team operated.
...
The starting point is that most working agreements are written once, in
the first weeks, because someone required one, and are never opened again. Everyone
...
Expect the agreement to be wrong within three months. The team that wrote it did not know
...
in months is not stable, it is unread.
...
less than the norms it operates under. The first two entries are that
...
- [How We Communicate](https://basecamp.com/guides/how-we-communicate), Basecamp: short, opinionated communication defaults worth adopting.
