...
Planning answers three questions in order: what could you build, what do you build first, and how will you know it worked. Each question has a well-worn set of methods, and most of them fit on an index card. The exercises that apply them to your project are in the [Creative](/activities/creative/) and [Planning](/activities/planning/) activities.
...
Of these rules, the quantity rule has the best support <Cite id="paulus-2018" form="parenthetical" />. <Cite id="paulus-2011" /> compared four brainstorming instructions (no specific focus, a quantity goal, a quality goal, or both) and found that the quantity goal produced more ideas, and more good ideas, than any of the other three.
...
The techniques below are structures that work around that loss, mostly by having people generate ideas alone or on paper before the group discusses them. Pick one per session.

If the team is remote, collect ideas in a shared document and then discuss them live: <Cite id="paulus-2018" /> report that electronic idea exchange tends to generate more ideas than talking, but that evaluating ideas works better face to face.
...
The one-minute limit forces breadth: you can't polish a sketch in a minute, so you move on to the next variation before committing to the first. The Design Sprint builds its whole sketching day on the same principle, which the authors call "work alone together". Crazy 8s works remotely with a shared whiteboard, or with paper held up to a camera.
...
<Cite id="karlsson-1997" /> developed a systematic cost-value approach for the usual situation of more requirements than time, and applied it to two commercial projects. The informal version is a matrix with cost on one axis (for a small team, almost always time) and the value delivered on the other:
...
- Cap the Must Haves at the effort the team is confident it can deliver. DSDM's typical ceiling is 60%.
...
- Re-rank when new information arrives, such as after a meeting with your project partner, rather than on a fixed schedule, because that's when an item's value changes.
