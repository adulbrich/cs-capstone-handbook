...
Most of the code in a Capstone project is now written with AI tools, and the course grades how far you delegated against how much your automation would catch, never the tooling itself. That automation is your **safety net**, and testing is most of it. The net has five parts, and each is checkable:
...
Automated tests should run on every pull request. If tests fail, the PR does not merge. This is the single most effective quality gate a team can implement. The workflow file and what else belongs in it are in the [DevOps guide](/guides/devops/#continuous-integration-ci); the branch protection that turns a red build into a blocked merge is in [Git and GitHub](/guides/git-and-github/#protecting-the-main-branch).
...
For accessibility, the common stack is [axe-core](https://github.com/dequelabs/axe-core) inside your test suite, [Lighthouse](https://developer.chrome.com/docs/lighthouse/) or [pa11y](https://pa11y.org/) for page-level budgets in CI, and a lint rule set such as [`eslint-plugin-jsx-a11y`](https://github.com/jsx-eslint/eslint-plugin-jsx-a11y) for JavaScript UI code. Screen readers for the manual pass are already on your machine: VoiceOver on macOS, [NVDA](https://www.nvaccess.org/) on Windows, Orca on Linux.
...
The tool names above date faster than anything else on this page. Last reviewed September 2026; reviewed each August.
:::
