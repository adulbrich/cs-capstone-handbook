...
2. **Whose work you reviewed, and the one change you asked for.** ([Google's code review standard](https://google.github.io/eng-practices/review/reviewer/standard.html) is the bar: approve when the change improves overall code health, and say what would have to change if it does not.) Name it specifically: "asked Priya to handle the empty-cart case in checkout" is a review; "LGTM" is not, and neither is a comment that names no line and requests no change.
