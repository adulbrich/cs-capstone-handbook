...
2. **Whose work you reviewed, and the one change you asked for.** ([Google's code review standard](https://google.github.io/eng-practices/review/reviewer/standard.html) is the bar: approve once the change improves the overall code health of the system, even when it is not perfect.) Name it specifically: "asked Priya to handle the empty-cart case in checkout" is a review; "LGTM" is not, and neither is a comment that names no line and requests no change.
