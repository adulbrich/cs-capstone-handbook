...
The criteria also ask for source integration with citations, so the RFC's Analysis item, its tradeoff criterion, the TSV band, and the template now say cited sources; the 2,000-word winter floor stays as local policy (the criteria ask for 1,500). The staff runbook gained a **WIC Compliance** table of the 13 criteria against the course as it runs. Three are open and one is partial: instructor-of-record feedback on a draft at least a week before the final (today the winter draft gets peer feedback and staff grade the final), the revision-after-instructor-feedback assessment that depends on it, GTA writing-feedback training, and the writing-volume and grade-weight accounting, whose numbers are in the runbook table and #60. Those are #60, waiting on the WIC director; the instructor's leaning is that the instructor samples the winter drafts and trained TAs cover the rest.
