...
#23 asked how bi-weekly check-ins line up with uniform sprint-note weeks; half of every cohort was meeting a week off the sprint boundary. The instructor decided, after a grilling pass: every check-in falls in the sprint-note week, on the day the TA sets, and the note and the meeting may come in either order (a note read first is the agenda, a demo seen first is what the note is checked against; a first draft fixed Monday to Thursday with the note due Friday, and the instructor cut that as a constraint the design does not need); note-less weeks keep a check-in (fall 2, spring 8 and 10), so every term has five check-in weeks and the ledger's 35 h holds; the defense moves out of the check-ins into a session of its own in week 7 or 9, the now-empty weeks, at about 7 h more per TA per term (ledger totals 69 / 70 / 64), because replacing the week-8 check-in would have turned every team's sprint-3 demo into a video; checkpoint 2 is graded in finals week so week 10 stays at check-in load; NDA walkthroughs take the team's nearest check-in, case by case, with no further specifics.

The same pass added the TA scaffolding the instructor asked for, after cohorts where TAs did not notice students doing no work (the same broken page shown at every check-in, "still working on it") and graded notes unevenly. The runbook gained **Check-in Format**: a five-minute prep list, an ordered agenda, a probe bank reusing the defense's question shapes, challenge moves for teams that are fine, a tells list, what to do (an evidence note into the modifier process, never a deduction of its own), and a one-line-per-student record in the cohort sheet; and **Sprint Note Checks**, the five clicks behind the pass/fail. The load-bearing rule is student-visible on the assignments overview and the sprint-note page: **the TA picks what each student demos, from that student's previous contribution line**, and the same artifact at two check-ins running is nothing new. Work that is not code (research, user research, requirements, RFC sections) is stated as valid on the sprint-note page, the template and the runbook, on one condition: it landed in the repository as a written PR someone can open. The Contribution lines rubric item now says graders open one owned PR per student and fails on one that does not open, did not land this sprint, or is mechanical, with no explanation; the sprint-note TSV mirrors it and needs re-importing.

## 21. WIC Outcomes Remapped (September 2026)

The WIC office replaced its three learning outcomes and its course criteria, and #59 asked for the new outcomes and a remap of every tag. The three verbatim sites (`LEARNING_OUTCOMES.json`, the WIC page, the CS 462 syllabus) carry the new sentences and the two "under official revision" placeholders are gone. The IDs stay `L07`-`L09`, so no validator logic, schema, or skill changed; what changed is the meaning behind each ID, so every tag was re-derived from the new sentence rather than moved slot by slot. `L07` (analyze disciplinary concepts through writing) now sits on the RFC's technical tradeoff analysis criterion, not on the peer reviews given, which stay `SO3`. `L08` (one original, extensive written work meeting disciplinary norms) and `L09` (integrate feedback after critical evaluation during revision) stay on the RFC's writing and revision criteria. The four team-page criteria that carried the old audience-and-genre `L08` or the old informal-writing `L07` (release report quality, landing page polish, project retrospective writing, term retrospective individual pages) are tagged `SO3` instead so the Outcome column stays complete; they are team pages and counted nothing either way. Coverage is unchanged at two individual data points per WIC outcome, both from the RFC.

The criteria also ask for source integration with citations, so the RFC's Analysis item, its tradeoff criterion, the TSV band, and the template now say cited sources; the 2,000-word winter floor stays as local policy (the criteria ask for 1,500). The staff runbook gained a **WIC Compliance** table, one row per criterion with the writing-volume criterion split into its four parts, against the course as it runs. Five rows are open and one is partial: the total-words, low-stakes-words, and share-of-grade parts of the writing-volume criterion, whose numbers are in the runbook table and #60; instructor-of-record feedback on a draft at least a week before the final (today the winter draft gets peer feedback and staff grade the final); the revision-after-instructor-feedback assessment that depends on it; and GTA writing-feedback training. Those are #60, waiting on the WIC director; the instructor's leaning is that the instructor samples the winter drafts and trained TAs cover the rest. The student-to-instructor ratio and the Writing Elevation prerequisite are for the WIC office to confirm.

## Build Status

Verified 2026-09-15, all gates green (section 21 landed).

`npm run build` passes: `astro check` reports **0 errors**, warnings only (the pre-existing `z` deprecation in `content.config.ts`). 67 pages built. Internal link validation is running: `starlightLinksValidator()` reports "All internal links are valid," and CI gates on it because CI runs `npm run build`.
...
`npm run check` (Biome via Ultracite core) passes on 23 code files.

`node scripts/validate-downloads.mjs` passes: 14 downloads in `public/`, every one linked from a handbook page.

The TSVs that **need re-importing into Canvas** are listed in `canvas/assignments/assignment-readme.md` (defense from section 14; project-retrospective, project-landing-page and career-retrospective from section 15; rfc, spring-release, project-landing-page and project-retrospective from section 21).
