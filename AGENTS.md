...
| `scripts/check-prose.mjs` | No em dash and no emoji in any tracked text file, and none of the glossary's rejected synonyms under the content paths. On handbook pages only, also no banned word, bolded whole sentence, or banned guide opener from `docs/agents/voice.md`; a guide carrying the legacy-opener marker skips the opener check until its sweep. Runs in CI, pre-commit, and the `after-edit` hook. |
